...
class cls_mysql {
	...
	function __construct($config=array()) {
		...
		$this->pconnect=isset($config['pconnect'])?$config['pconnect']:0;
		$this->quiet=isset($config['quiet'])?$config['quiet']:0;
		$this->charset=isset($config['charset'])?$config['charset']:'utf8';
		$this->dbhost=isset($config['dbhost'])?$config['dbhost']:'localhost';
		$this->cache_data_dir=isset($config['cache_data_dir'])?$config['cache_data_dir']:str_replace("\\", '/', substr(str_replace("\\", '/', dirname(__FILE__) ),0) ).'/data/wdcache/sqlcache/';
		if ($this->quiet) {
			$this -> connect($this->dbhost, $this->dbuser, $this->dbpw, $this->dbname, $this->charset, $this->pconnect, $this->quiet);
		} else {
			$this -> settings = array('dbhost' => $this->dbhost, 'dbuser' => $this->dbuser, 'dbpw' => $this->dbpw, 'dbname' => $this->dbname, 'charset' => $this->charset, 'pconnect' => $this->pconnect);
		}
	}
	...
	function getOneCached($sql, $cached = 'MYSQLFIRST') {
	...
	function getAllCached($sql, $cached = 'MYSQLFIRST') {
		$result = $this -> getSqlCacheData($sql, $cached);
		if (empty($result['storecache']) == true) {
			return $result['data'];
		}
	...
	function getRowCached($sql, $cached = 'MYSQLFIRST') {
		$sql = trim($sql . ' LIMIT 1');
		$result = $this -> getSqlCacheData($sql, $cached);
		if (empty($result['storecache']) == true) {
			return $result['data'];
	...
	function getColCached($sql, $cached = 'MYSQLFIRST') {
		$result = $this -> getSqlCacheData($sql, $cached);
		if (empty($result['storecache']) == true) {
			return $result['data'];
		}
	...
	function getSqlCacheData($sql, $cached = '') {
		$sql = trim($sql);
		$pathstr = md5($this -> dbhash . $sql);
		$pathstr1 = $pathstr{0} . $pathstr{1} . "/" . $pathstr{2} . $pathstr{3} . "/";
		$this->mkdirs($this -> root_path . $this -> cache_data_dir . $pathstr1);
		$result = array();
		$result['filename'] = $this -> root_path . $this -> cache_data_dir . $pathstr1 . 'sqlcache_' . abs(crc32($this -> dbhash . $sql)) . '_' . md5($this -> dbhash . $sql) . '.php';
	...
	function get_table_name($query_item) {
		...
		$prefix = "#@_";
		$query_item = str_replace($prefix, $this->dbprefix, $query_item);
		$table_names = array();
